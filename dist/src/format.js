...
function maskEmailPartPrefix(value, visibleChars) {
...
function maskEmailDomain(value) {
    ...
    const publicSuffix = parts.length > 1 ? `.${parts.slice(domainNameIndex + 1).join(".")}` : "";
...
function formatThreadButtonTitle(thread) {
...
function isLikelyWorktreePath(value) {
...
export function formatThreadPicker(threads) {
    if (threads.length === 0) {
...
export function formatThreadButtonLabel(params) {
    ...
    const reservedLength = (iconPrefix ? `${iconPrefix} `.length : 0) +
...
export function formatProjectPickerIntro(params) {
    const scopeLabel = params.action === "start-new-thread"
        ? params.workspaceDir
...
export function formatThreadState(state, binding) {
    return [
...
export function formatCodexAccountText(account) {
    if (!account) {
        return "unknown";
    }
...
export function formatCodexModelText(threadState) {
...
function formatCodexFastModeValue(value) {
...
export function getCodexStatusTimeZoneLabel() {
...
export function formatCodexRateLimitLine(limit, nowMs = Date.now()) {
...
function splitCodexRateLimitName(name) {
...
export function formatCodexContextUsageSnapshot(usage) {
    if (!usage) {
        return undefined;
    }
    ...
    const percentFull = typeof totalTokens === "number" && typeof contextWindow === "number" && contextWindow > 0
    ...
    return `${totalLabel} / ${contextLabel} tokens used${extras.length > 0 ? ` (${extras.join(", ")})` : ""}`;
}
export function formatCodexStatusText(params) {
    ...
    lines.push(params.bindingActive
...
export function formatBoundThreadSummary(params) {
    ...
    const projectName = getProjectName(workspacePath) ||
        ...
        "Unknown";
    const threadName = params.state?.threadName?.trim() ||
...
export function formatAccountSummary(account, limits) {
...
export function formatModels(models, state) {
    if (models.length === 0) {
    ...
    lines.push("Available models:", ...models.slice(0, 20).map((model) => {
        const current = model.id === state?.model || (!state?.model && model.current) ? " (current)" : "";
...
export function formatSkills(params) {
    ...
    for (const skill of skills.slice(0, 20)) {
        ...
        const state = skill.enabled === false ? " (disabled)" : skill.enabled === true ? "" : " (status unknown)";
...
export function filterSkillsByQuery(skills, filter) {
...
export function formatSkillHelpText(skill) {
...
export function formatExperimentalFeatures(features) {
    if (features.length === 0) {
        return "No Codex experimental features reported.";
    }
    return [
        "Codex experimental features:",
        ...features.slice(0, 30).map((feature) => `- ${feature.displayName || feature.name}${feature.enabled ? " (enabled)" : ""}`),
    ].join("\n");
}
...
export function formatThreadReplay(replay) {
    return [
...
export function formatTurnCompletion(result) {
...
export function formatReviewCompletion(result) {
    return result.reviewText.trim() || (result.aborted ? "Codex review stopped." : "Codex review completed.");
}
export function parseCodexReviewOutput(text) {
    const lines = text.trim().split(/\r?\n/);
    const findings = [];
    const summaryLines = [];
    const findingRe = /^-?\s*(?:\[(?<priority>P\d)\]\s*)?(?<title>.+?)(?:\s+Location:\s*(?<location>.+))?$/i;
    let current = null;
    ...
    for (const line of lines) {
        ...
        const looksLikeFinding = (trimmed.startsWith("[P") || trimmed.startsWith("- [P")) &&
...
export function formatCodexPlanSteps(steps) {
...
export function formatCodexPlanInlineText(plan) {
    const lines = ["Plan"];
...
export function buildCodexPlanMarkdownPreview(markdown, maxChars = 1400) {
...
export function formatCodexPlanAttachmentSummary(plan) {
...
export function formatCodexPlanAttachmentFallback(plan) {
    const lines = [
    ...
    if (markdownPreview) {
        ...
        const preview = markdownPreview.length > maxPreviewChars
