...
function isFileChangeApprovalMethod(methodLower) {
    return methodLower.includes("filechange/requestapproval");
}
function isCommandApprovalMethod(methodLower) {
...
function humanizeApprovalDecision(decision, sessionPrefix) {
    switch (decision) {
        case "accept":
            return "Approve Once";
        case "acceptForSession":
...
function extractSessionPrefix(value) {
    const record = asRecord(value);
    return (findFirstStringByKeys(record?.proposedExecpolicyAmendment, [
        "prefix",
        "commandPrefix",
        "prefixToApprove",
        "allowedPrefix",
...
function buildApprovalActionsFromDecisions(value) {
    const record = asRecord(value);
    ...
    for (const entry of rawDecisions) {
        ...
        const decisionValue = pickString(decisionRecord, ["decision", "value", "name", "id", "action"]) ?? "";
        const decision = normalizeApprovalDecision(decisionValue);
        if (!decision) {
            continue;
        }
        const sessionPrefix = decision === "acceptForSession" ? extractSessionPrefix(decisionRecord) : undefined;
        const proposedExecpolicyAmendment = decision === "acceptForSession"
        ...
        actions.push({
            ...
            label: pickString(decisionRecord, ["label", "title", "text"]) ??
                humanizeApprovalDecision(decision, sessionPrefix),
        });
    }
    return actions;
}
function resolveApprovalDecisionFromText(text) {
...
function buildApprovalActionsFromOptions(options) {
    const seen = new Set();
    const actions = [];
...
function buildApprovalActionsFromMethod(methodLower, requestParams) {
...
export function buildPendingUserInputActions(params) {
    ...
    if (methodLower.includes("requestapproval")) {
        ...
        const resolvedApprovalActions = approvalActions.length > 0
            ? approvalActions
...
function truncateWithNotice(text, maxChars, notice) {
...
function parseQuestionnaireOption(line) {
...
function extractQuestionnaireFromStructuredRequest(value) {
    const record = asRecord(value);
...
export function parsePendingQuestionnaire(text) {
...
export function formatPendingQuestionnairePrompt(questionnaire) {
    const question = questionnaire.questions[questionnaire.currentIndex];
    if (!question) {
        return "Codex needs input.";
    }
    const heading = question.header && question.prompt && question.header !== question.prompt
    ...
    if (currentAnswer) {
        lines.push("", `Current answer: ${currentAnswer.kind === "option"
...
export function renderPendingQuestionnaireAnswer(answer) {
    if (!answer) {
        return "";
    }
    return answer.kind === "option" ? answer.optionLabel.trim() : answer.text.trim();
}
export function buildPendingQuestionnaireResponse(questionnaire) {
    ...
    return {
        answers: Object.fromEntries(questionnaire.questions.map((question, index) => {
            const answer = questionnaire.answers[index];
...
export function addQuestionnaireResponseNote(response, note) {
...
export function questionnaireIsComplete(questionnaire) {
    return questionnaire.answers.every((answer) => answer != null &&
        (answer.kind === "option" || (answer.kind === "text" && answer.text.trim().length > 0)));
}
export function questionnaireCurrentQuestionHasAnswer(questionnaire) {
...
function buildMarkdownCodeBlock(text, language = "") {
    const normalized = text.replace(/\r\n/g, "\n").trim();
    if (!normalized) {
        return "";
    }
...
export function stripShellLauncher(command) {
    const match = command.match(/^(?:\/[/\w]*\/)?(?:bash|zsh|sh|dash|ksh|tcsh|fish)\s+-lc\s+(['"])([\s\S]*)\1\s*$/);
    if (match) {
        return match[2];
    }
    return command;
 ...
 */
export function extractCommandFromActions(requestParams) {
...
export function buildPendingPromptText(params) {
    ...
    if (requestText) {
        lines.push(truncateWithNotice(requestText, MAX_PENDING_REQUEST_TEXT_CHARS, "[Request details truncated. Use steer text if you want to redirect Codex.]"));
    }
    ...
    const rawCommand = findFirstStringByKeys(params.requestParams, [
        "command",
        "cmd",
        "displayCommand",
        "rawCommand",
    ...
    return truncateWithNotice(lines.join("\n"), MAX_PENDING_PROMPT_TEXT_CHARS, "[Prompt truncated for chat delivery. Use the buttons or reply with steer text.]");
}
export function createPendingInputState(params) {
    ...
    const questionnaire = extractQuestionnaireFromStructuredRequest(params.requestParams) ??
...
export function parseCodexUserInput(text, optionsCount) {
...
export function requestToken(requestId) {
    return crypto.createHash("sha1").update(requestId).digest("base64url").slice(0, 10);
}
//# sourceMappingURL=pending-input.js.map