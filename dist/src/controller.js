...
import { CodexAppServerModeClient, isMissingThreadError } from "./client.js";
import { getThreadDisplayTitle } from "./thread-display.js";
import { formatBoundThreadSummary, formatCodexPlanAttachmentFallback, formatCodexPlanAttachmentSummary, formatCodexPlanInlineText, formatCodexReviewFindingMessage, formatCodexStatusText, formatExperimentalFeatures, formatSkillHelpText, formatSkillsPickerText, filterSkillsByQuery, formatMcpServers, formatModels, parseCodexReviewOutput, formatProjectPickerIntro, formatSkills, formatThreadButtonLabel, formatThreadPickerIntro, formatTurnCompletion, } from "./format.js";
import { formatReasoningEffortLabel, getSupportedReasoningEfforts, modelSupportsFast, normalizeReasoningEffort, REASONING_EFFORT_OPTIONS, } from "./model-capabilities.js";
import { formatCommandUsage, renderCommandHelpText } from "./help.js";
import { addQuestionnaireResponseNote, buildPendingQuestionnaireResponse, formatPendingQuestionnairePrompt, questionnaireCurrentQuestionHasAnswer, questionnaireIsComplete, requestToken, } from "./pending-input.js";
import { buildConversationKey, buildPluginSessionKey, PluginStateStore, } from "./state.js";
import { parseThreadSelectionArgs, expandHomeDir, selectThreadFromMatches, } from "./thread-selection.js";
import { filterThreadsByProjectName, getProjectName, listProjects, paginateItems, } from "./thread-picker.js";
import { INTERACTIVE_NAMESPACE, PLUGIN_ID, } from "./types.js";
import { isMissingPluginSdkSubpathError, loadOpenClawCompatModule, resolveOpenClawEntrypointPath, resolveCompatFallbackPath, } from "./openclaw-sdk-compat.js";
...
function asScopedBindingApi(value) {
    return value;
}
...
function denormalizeDiscordConversationId(raw) {
    if (!raw) {
        return undefined;
    }
    const trimmed = raw.trim();
...
function readCommandContextId(ctx, key) {
...
function normalizeDiscordChannelConversationId(raw) {
...
function resolveDiscordCommandConversation(ctx) {
    const threadConversationId = normalizeDiscordChannelConversationId(readCommandContextId(ctx, "messageThreadId"));
    if (threadConversationId) {
        return {
            channel: "discord",
            accountId: ctx.accountId ?? "default",
            conversationId: threadConversationId,
            parentConversationId: normalizeDiscordChannelConversationId(readCommandContextId(ctx, "threadParentId")),
        };
    }
...
function toConversationTargetFromInbound(event) {
    ...
    const conversationId = channel === "telegram"
        ...
        : channel === "discord"
            ? (() => {
                ...
                const guildId = typeof event.metadata?.guildId === "string" ? event.metadata.guildId.trim() : "";
    ...
    const parentConversationId = channel === "telegram" && numericThreadId != null
...
function normalizeInboundMediaPath(value) {
    const trimmed = value?.trim();
    if (!trimmed) {
        return undefined;
    }
...
function extractInboundMetadataMedia(metadata) {
    if (!metadata) {
        return [];
    }
    ...
    for (let index = 0; index < count; index += 1) {
        ...
        results.push({
            kind: isImageMimeType(mimeType) || isImagePathLike(normalizedPath)
                ? "image"
                : "document",
...
function toCodexImageInputItem(media) {
    if (media.kind !== "image" &&
...
async function toCodexTextAttachmentInputItem(media) {
    if (media.kind === "image" ||
...
async function buildInboundTurnInput(event) {
    ...
    for (const media of [...(event.media ?? []), ...extractInboundMetadataMedia(event.metadata)]) {
        ...
        const key = item.type === "localImage"
            ? `${item.type}:${item.path}`
            : item.type === "image"
                ? `${item.type}:${item.url}`
                : `${item.type}:${item.text}`;
...
function isQueueCompatibleTurnInput(prompt, input) {
    if (!input?.length) {
        return true;
    }
    return input.length === 1 && input[0]?.type === "text" && input[0].text === prompt;
}
function buildReplyWithButtons(text, buttons) {
    return buttons
        ? {
            text,
            channelData: {
...
function extractReplyButtons(reply) {
    ...
    for (const block of blocks) {
        ...
        const row = buttons
            ...
            const callbackData = typeof buttonRecord?.value === "string" ? buttonRecord.value.trim() : "";
            if (!text || !callbackData) {
                return null;
            }
            const style = buttonRecord.style === "danger" ||
...
function parseFastAction(argsText) {
...
function formatFastModeValue(value) {
...
function normalizePreferenceServiceTier(value) {
    const normalized = normalizeServiceTier(value);
...
function requestServiceTierFromPreference(value) {
...
function preferredServiceTierFromRequest(value) {
    return normalizePreferenceServiceTier(value) ?? "default";
}
function getPermissionsForMode(profile) {
...
function formatPermissionsModeLabel(profile) {
    return profile === "full-access" ? "Full Access" : "Default";
}
function applyBindingPreferencesToThreadState(threadState, binding) {
    if (!threadState && !binding) {
        return undefined;
    }
    const preferredModel = binding?.preferences?.preferredModel?.trim();
    const preferredServiceTier = normalizePreferenceServiceTier(binding?.preferences?.preferredServiceTier);
    const preferredReasoningEffort = normalizeReasoningEffort(binding?.preferences?.preferredReasoningEffort);
...
function buildDesiredThreadConfiguration(threadState, binding, modelFallback) {
...
function formatThreadStateForLog(threadState) {
    if (!threadState) {
...
function formatBindingPreferencesForLog(binding) {
    return [
...
function buildPermissionsUnavailableNote() {
...
function buildPendingPermissionsMigrationNote(profile) {
...
function isTransportClosedMessage(error) {
...
function formatFailureText(kind, error) {
...
function formatInterruptedText(kind) {
    return `Codex ${kind} was interrupted before it finished.`;
}
function formatContextUsageText(usage) {
    ...
    const context = typeof usage.contextWindow === "number"
        ? usage.contextWindow >= 1000
            ? `${(usage.contextWindow / 1000).toFixed(usage.contextWindow >= 10000 ? 0 : 1)}k`
            : String(usage.contextWindow)
        : "?";
    const percent = typeof usage.contextWindow === "number" && usage.contextWindow > 0
...
function hasCommandPreferenceOverrides(overrides) {
    return (typeof overrides.requestedFast === "boolean" ||
        typeof overrides.requestedYolo === "boolean" ||
        Boolean(overrides.requestedModel?.trim()));
}
function mergeConversationPreferences(existing, updates) {
...
function normalizePreferencesForModel(preferences, model) {
    if (!preferences) {
        return preferences;
    }
...
function buildResumeTopicName(params) {
...
function formatThreadSelectionFlags(parsed) {
    return [
...
function buildThreadOnlyName(params) {
    const threadName = params.title?.trim() || params.threadId.trim();
    const projectName = path.basename(params.projectKey?.replace(/[\\/]+$/, "").trim() || "");
    return normalizeThreadTitleProjectSuffix(threadName, projectName) || undefined;
}
function normalizeThreadTitleProjectSuffix(threadName, projectName) {
...
function truncateDiscordLabel(text, maxChars = 80) {
...
function listWorkspaceChoices(threads, projectName) {
...
export class CodexPluginController {
    ...
    client;
    activeRuns = new Map();
    threadChangesCache = new Map();
    store;
    serviceWorkspaceDir;
    lastRuntimeConfig;
    telegramMenuRepairHandle;
    started = false;
    constructor(api) {
        this.api = api;
    ...
    async handleConversationBindingResolved(event) {
        await this.start();
        const conversation = {
        ...
        if (!pending) {
            this.api.logger.debug?.(`codex binding approved without pending local bind conversation=${conversation.conversationId}`);
            return;
        }
    ...
    async handleInboundClaim(event) {
        try {
            ...
            if (active) {
                if (active.mode === "plan") {
                    this.api.logger.debug?.(`codex inbound claim restarting active plan run conversation=${conversation.conversationId}`);
                    ...
                    if (pending?.state.questionnaire && !event.content.trim().startsWith("/")) {
                        const handled = await this.handlePendingQuestionnaireFreeformAnswer(conversation, pending, active.handle, event.content);
                        if (handled) {
                            return { handled: true };
                        }
                    }
                    if (requiresStructuredInput) {
                        this.api.logger.debug?.(`codex inbound claim restarting active run for structured input conversation=${conversation.conversationId}`);
                    }
                    else {
                        try {
                            ...
                            this.api.logger.warn(`codex inbound claim could not enqueue message for active run; restarting thread conversation=${conversation.conversationId}`);
                        }
                        catch (error) {
                            this.api.logger.warn(`codex inbound claim active run enqueue failed; restarting thread conversation=${conversation.conversationId}: ${String(error)}`);
                        }
                    }
                    this.activeRuns.delete(activeKey);
                    await active.handle.interrupt().catch(() => undefined);
            ...
            this.api.logger.debug?.(`codex inbound claim channel=${conversation.channel} account=${conversation.accountId} conversation=${conversation.conversationId} parent=${conversation.parentConversationId ?? "<none>"} local=${resolvedBinding ? "yes" : "no"}`);
            if (!resolvedBinding) {
                return { handled: false };
            }
            ...
            this.api.logger.debug?.(`codex inbound claim starting turn ${this.formatConversationForLog(conversation)} workspace=${resolvedBinding.workspaceDir} thread=${resolvedBinding.threadId} prompt="${summarizeTextForLog(event.content)}"`);
            await this.startTurn({
                conversation,
            ...
            this.api.logger.debug?.(`codex inbound claim turn accepted ${this.formatConversationForLog(conversation)}`);
            return { handled: true };
        }
        catch (error) {
    ...
    async handleTelegramInteractive(ctx) {
        await this.start();
        ...
        await this.dispatchCallbackAction(callback, {
            ...
            sourceMessage: ctx.callback.messageId != null && ctx.callback.chatId?.trim()
                ? {
                    provider: "telegram",
            ...
            requestConversationBinding: async (params) => {
                ...
                if (!requestConversationBinding) {
                    return { status: "error", message: "Conversation binding is unavailable." };
                }
                ...
                if (result.status === "pending") {
                    ...
                    return { status: "pending", reply: result.reply };
                }
                return result;
            },
    ...
    async handleDiscordInteractive(ctx) {
        await this.start();
        const bindingApi = asScopedBindingApi(ctx);
        ...
        const callbackConversationId = callback.conversation.channel === "discord"
            ? normalizeDiscordConversationId(callback.conversation.conversationId)
            : undefined;
        const conversationId = callbackConversationId ??
        ...
        try {
            ...
            await this.dispatchCallbackAction(callback, {
                ...
                clear: async () => {
                    ...
                    if ((callback.kind === "pending-input" || callback.kind === "pending-questionnaire") && messageId) {
                        await ctx.respond
                            ...
                            .catch(() => undefined);
                        const completionText = callback.kind === "pending-questionnaire"
                            ? "Recorded your answers and sent them to Codex."
                            : "Sent to Codex.";
                        await this.editDiscordComponentMessage(conversation.conversationId, messageId, {
                            text: completionText,
                        }, {
                            accountId: conversation.accountId,
                        }).catch((error) => {
                            this.api.logger.warn(`codex discord ${callback.kind} clear failed conversation=${conversationId}: ${String(error)}`);
                        });
                        return;
                    }
                    try {
                ...
                editPicker: async (picker) => {
                    this.api.logger.debug(`codex discord picker refresh conversation=${conversationId} rows=${picker.buttons?.length ?? 0}`);
                    const messageId = ctx.interaction.messageId?.trim();
                    ...
                    if (builtPicker) {
                        try {
                            ...
                            if (!messageId) {
                                this.api.logger.warn(`codex discord picker edit failed conversation=${conversationId}: ${detail}`);
                            }
                    ...
                    if (messageId) {
                        try {
                            ...
                            await this.editDiscordComponentMessage(conversation.conversationId, messageId, this.buildDiscordPickerSpec(picker), {
                                accountId: conversation.accountId,
                            });
                            return;
                        }
                        catch (error) {
                            this.api.logger.warn(`codex discord picker edit failed conversation=${conversationId}: ${String(error)}`);
                        }
                    }
                    try {
                        ...
                        this.api.logger.warn(`codex discord picker send failed conversation=${conversationId}: ${String(error)}`);
                        throw error;
                    }
                },
                requestConversationBinding: async (params) => {
                    ...
                    if (result.status === "pending") {
                        ...
                        if (callback.kind === "resume-thread" && originalMessageId) {
                            await this.editDiscordComponentMessage(conversation.conversationId, originalMessageId, {
    ...
    async handleCommand(commandName, ctx) {
        ...
        const currentBinding = conversation && bindingApi.getCurrentConversationBinding
        ...
        const hydratedBinding = conversation && currentBinding && !existingBinding
        ...
        if (isDiscordChannel(ctx.channel)) {
            this.api.logger.debug(`codex discord command /${commandName} from=${ctx.from ?? "<none>"} to=${ctx.to ?? "<none>"} conversation=${conversation?.conversationId ?? "<none>"}`);
        }
        switch (commandName) {
            case "cas_resume":
                return await this.handleJoinCommand(conversation, binding, args, ctx.channel, ctx, pendingBind, hydratedBinding?.pendingBind);
            case "cas_detach":
                if (!conversation) {
            ...
            case "cas_status":
                return await this.handleStatusCommand(conversation, binding, args, Boolean(currentBinding || binding));
            case "cas_stop":
            ...
            case "cas_permissions":
                return await this.handlePermissionsCommand(conversation, binding, Boolean(currentBinding || binding));
            case "cas_init":
    ...
    async handleStartNewThreadSelection(conversation, binding, parsed, channel, requestConversationBinding) {
        if (!conversation) {
            return { text: "This command needs a Telegram or Discord conversation." };
        }
        ...
        const result = await this.startNewThreadAndBindConversation(conversation, binding, workspaceDir, parsed.syncTopic, {
    ...
    async handleListCommand(conversation, binding, filter, channel) {
    ...
    async handleJoinCommand(conversation, binding, args, channel, ctx, pendingBind, hydratedPendingBind) {
        const bindingApi = asScopedBindingApi(ctx);
        if (!conversation) {
            return { text: "This command needs a Telegram or Discord conversation." };
        }
        ...
        if (parsed.startNew) {
            return await this.handleStartNewThreadSelection(conversation, binding, parsed, channel, bindingApi.requestConversationBinding);
        }
        if (hydratedPendingBind?.notifyBound &&
            !parsed.listProjects &&
            !parsed.query) {
        ...
        if (pendingBind && !binding && !parsed.listProjects && !parsed.query) {
            ...
            const targetPermissionsMode = this.resolveRequestedPermissionsMode(normalizePermissionsMode(pendingBind.permissionsMode), parsed.requestedYolo);
            const preferences = this.buildBindingPreferencesWithOverrides(pendingBind.preferences, overrides, parsed.requestedModel);
        ...
        const selection = await this.resolveSingleThread(binding?.sessionKey, workspaceDir, parsed.query);
        ...
        const targetPermissionsMode = this.resolveRequestedPermissionsMode(this.getPermissionsMode(binding), parsed.requestedYolo);
        const preferences = this.buildBindingPreferencesWithOverrides(binding?.preferences, overrides, parsed.requestedModel);
        const bindResult = await this.requestConversationBinding(conversation, {
            threadId: selection.thread.threadId,
            workspaceDir: selection.thread.projectKey ||
                workspaceDir ||
                resolveWorkspaceDir({
    ...
    async handleStatusCommand(conversation, binding, args, bindingActive) {
        ...
        if (hasCommandPreferenceOverrides(overrides)) {
            ...
            const effectiveModel = parsed.requestedModel?.trim() ||
            ...
            const targetPermissionsMode = this.resolveRequestedPermissionsMode(currentPermissionsMode, parsed.requestedYolo);
            ...
            const nextPreferences = this.buildBindingPreferencesWithOverrides(binding.preferences, overrides, effectiveModel);
            const updatedBindingBase = {
                ...binding,
            ...
            binding =
                targetPermissionsMode !== currentPermissionsMode
                    ? active
                        ? await this.persistBindingPermissionsMode(updatedBindingBase, currentPermissionsMode, targetPermissionsMode)
                        : currentThreadState
            ...
            await this.reconcileThreadConfiguration(binding, {
                applyPermissions: Boolean(currentThreadState) && !(active && targetPermissionsMode !== currentPermissionsMode),
    ...
    hasFullAccessProfile() {
        return this.client.hasProfile("full-access");
    }
    getPermissionsMode(binding) {
        return getBindingPermissionsMode(binding ?? null);
    }
    async waitForActiveRunToClear(conversation, timeoutMs = 3_000) {
    ...
    async clearPendingRequestForConversation(conversation, reason) {
        const pending = this.store.getPendingRequestByConversation(conversation);
        if (!pending) {
            return false;
        }
        await this.store.removePendingRequest(pending.requestId);
        this.api.logger.debug?.(`codex cleared pending request ${this.formatConversationForLog(conversation)} requestId=${pending.requestId} reason=${reason}`);
        return true;
    }
    async clearStaleInteractiveRun(conversation, reason) {
        const key = buildConversationKey(conversation);
        if (this.activeRuns.delete(key)) {
            this.api.logger.warn(`codex dropped stale interactive run ${this.formatConversationForLog(conversation)} reason=${reason}`);
        }
    ...
    async syncBindingPermissionsModeFromThreadState(binding, threadState, context) {
        ...
        this.api.logger.warn(`codex refreshed binding permissions mode from live thread state ${this.formatConversationForLog(conversation)} stored=${storedProfile} live=${liveProfile} context=${context}`);
        return {
            binding: nextBinding,
            note: `Permissions note: refreshed the stored mode from the live ${formatPermissionsModeLabel(liveProfile)} thread state.`,
        };
    }
    async readEffectiveThreadState(binding) {
        ...
        const syncedBinding = state
            ? (await this.syncBindingPermissionsModeFromThreadState(binding, state, "read effective thread state")).binding
            : binding;
    ...
    async resolveCurrentModelHint(binding, effectiveState) {
        const explicitModel = effectiveState?.model?.trim() || binding.preferences?.preferredModel?.trim() || undefined;
        if (explicitModel) {
    ...
    resolveRequestedPermissionsMode(currentProfile, requestedYolo) {
        if (requestedYolo === undefined) {
            return currentProfile;
        }
        return requestedYolo ? "full-access" : "default";
    }
    buildPreferenceUpdatesFromOverrides(overrides) {
        const updates = {};
    ...
    buildBindingPreferencesWithOverrides(existing, overrides, modelHint) {
        return normalizePreferencesForModel(mergeConversationPreferences(existing, this.buildPreferenceUpdatesFromOverrides(overrides)), overrides.requestedModel?.trim() || modelHint);
    }
    async reconcileThreadConfiguration(binding, opts) {
        const profile = this.getPermissionsMode(binding);
        let state = opts?.threadState ??
        ...
        if (desired.model && desired.model !== state?.model?.trim()) {
            try {
                ...
                this.api.logger.warn(`codex failed to ${opts?.context ?? "reconcile thread settings"} model: ${String(error)}`);
            }
        }
        ...
        if (desiredServiceTier !== currentServiceTier) {
            try {
                ...
                this.api.logger.warn(`codex failed to ${opts?.context ?? "reconcile thread settings"} fast mode: ${String(error)}`);
            }
        }
        if (opts?.applyPermissions !== false &&
            ...
            try {
                ...
                this.api.logger.warn(`codex failed to ${opts?.context ?? "reconcile thread settings"} permissions: ${String(error)}`);
            }
        }
        return state;
    }
    async buildStatusControlButtons(conversation, binding) {
        ...
        const currentReasoning = normalizeReasoningEffort(effectiveState?.reasoningEffort ?? binding.preferences?.preferredReasoningEffort);
    ...
    async sendReplyPayloadToConversation(conversation, payload) {
    ...
    async sendPickerToConversation(conversation, picker) {
        await this.sendText(conversation, picker.text, { buttons: picker.buttons });
    }
    async updateStatusCardMessage(conversation, message, statusCard) {
        try {
            ...
            await this.editDiscordComponentMessage(message.channelId, message.messageId, this.buildDiscordPickerSpec({
                text: statusCard.text,
                buttons: statusCard.buttons,
            }), {
                accountId: conversation.accountId,
            ...
            this.api.logger.warn(`codex status card update failed ${this.formatConversationForLog(conversation)} provider=${message.provider}: ${String(error)}`);
            return false;
        }
    }
    async buildModelPicker(conversation, binding, opts) {
        const profile = this.getPermissionsMode(binding);
        ...
        this.api.logger.debug?.(`codex model picker conversation=${this.formatConversationForLog(conversation)} raw=${formatThreadStateForLog(state)} effective=${formatThreadStateForLog(effectiveState)} ${formatBindingPreferencesForLog(binding)}`);
        const buttons = [];
        for (const model of models.slice(0, 8)) {
            ...
            buttons.push([
                {
                    text: `${model.id}${model.id === effectiveState?.model || (!effectiveState?.model && model.current)
                        ? " (current)"
                        : ""}`,
    ...
    async buildReasoningPicker(conversation, binding, opts) {
        ...
        if (!model) {
            return {
                text: "Select a model first. Reasoning choices will also become available after Codex materializes the thread.",
                buttons: [],
            };
        }
    ...
    async buildStatusCard(conversation, binding, bindingActive) {
    ...
    async buildSkillsPicker(conversation, binding, opts) {
        ...
        for (let index = 0; index < paged.items.length; index += 2) {
            ...
            const row = await Promise.all(pair.map(async (skill) => {
                const callback = opts.clickMode === "run"
    ...
    async handleStopCommand(conversation) {
        if (!conversation) {
            return { text: "This command needs a Telegram or Discord conversation." };
        }
        const key = buildConversationKey(conversation);
        const active = this.activeRuns.get(buildConversationKey(conversation));
        if (!active) {
            const cleared = await this.clearPendingRequestForConversation(conversation, "stop command without active run");
            if (cleared) {
                await this.applyPendingBindingPermissionsModeMigration(conversation);
    ...
    async handleSteerCommand(conversation, args) {
        if (!conversation) {
            return { text: "This command needs a Telegram or Discord conversation." };
        }
    ...
    async handlePlanCommand(conversation, binding, args) {
        if (!conversation) {
            return { text: "This command needs a Telegram or Discord conversation." };
        }
        ...
        if (parsed.mode === "off") {
            ...
            this.api.logger.debug?.(`codex plan off requested ${this.formatConversationForLog(conversation)} active=${active?.mode ?? "none"} boundThread=${binding?.threadId ?? "<none>"}`);
            if (active?.mode === "plan") {
        ...
        return buildPlainReply("Starting Codex plan mode. I’ll relay the questions and final plan as they arrive.");
    }
    async handleReviewCommand(conversation, binding, args) {
        ...
        return buildPlainReply(args.trim()
    ...
    async handleCompactCommand(conversation, binding) {
        if (!conversation || !binding) {
    ...
    async startCompact(params) {
        ...
        try {
            ...
            const progressTimer = setTimeout(() => {
                void (async () => {
                    const usageText = latestUsage ? formatContextUsageText(latestUsage) : undefined;
                    if (usageText && usageText !== lastEmittedUsageText) {
                        lastEmittedUsageText = usageText;
                    }
                    await this.sendText(conversation, usageText
            ...
            await this.sendText(conversation, [
    ...
    async handleSkillsCommand(conversation, binding, args) {
        const workspaceDir = resolveWorkspaceDir({
            bindingWorkspaceDir: binding?.workspaceDir,
            configuredWorkspaceDir: this.settings.defaultWorkspaceDir,
            serviceWorkspaceDir: this.serviceWorkspaceDir,
    ...
    async handleExperimentalCommand(binding) {
    ...
    async handleMcpCommand(binding, args) {
    ...
    async handleFastCommand(binding, args) {
        if (!binding) {
        ...
        const currentModel = effectiveState?.model?.trim() || binding.preferences?.preferredModel?.trim() || undefined;
        if (!modelSupportsFast(currentModel)) {
            return {
        ...
        const nextTier = action === "toggle" ? (currentTier === "fast" ? null : "fast")
            : action === "on" ? "fast"
                : null;
    ...
    async handleModelCommand(conversation, binding, args) {
        ...
        const nextPreferredServiceTier = modelSupportsFast(trimmedArgs)
            ...
            : "default";
        const nextState = !modelSupportsFast(trimmedArgs) && normalizeServiceTier(state.serviceTier) === "fast"
            ? await this.client
    ...
    async handlePermissionsCommand(conversation, binding, bindingActive) {
        return await this.handleStatusCommand(conversation, binding, "", bindingActive);
    }
    async handlePromptAlias(conversation, binding, args, alias, ctx) {
        if (!conversation) {
            return { text: "This command needs a Telegram or Discord conversation." };
        }
        ...
        if (parsed.cwd) {
            ...
            const result = await this.startNewThreadAndBindConversation(conversation, binding, parsed.cwd, false, {}, bindingApi.requestConversationBinding, false, prompt);
            if (result.status === "pending") {
                return result.reply;
            }
            if (result.status === "error") {
    ...
    async handleRenameCommand(conversation, binding, args) {
        if (!conversation || !binding) {
    ...
    async buildRenameStylePicker(conversation, binding, syncTopic) {
        const profile = this.getPermissionsMode(binding);
    ...
    async applyRenameStyle(conversation, binding, style, syncTopic) {
        const profile = this.getPermissionsMode(binding);
        const threadState = await this.client
            .readThreadState({
            profile,
            ...
            .catch(() => undefined);
        const name = style === "thread-project"
    ...
    async startTurn(params) {
        ...
        if (binding) {
            ...
            if (threadState) {
                binding = (await this.syncBindingPermissionsModeFromThreadState(binding, threadState, `start turn (${params.reason})`)).binding;
            }
        }
        const profile = this.getPermissionsMode(binding);
        const existing = this.activeRuns.get(key);
        this.api.logger.debug?.(`codex turn request reason=${params.reason} ${this.formatConversationForLog(params.conversation)} workspace=${params.workspaceDir} existing=${existing ? existing.mode : "none"} profile=${profile} prompt="${summarizeTextForLog(params.prompt)}"`);
        if (existing) {
            if (existing.mode === "plan" && (params.collaborationMode?.mode ?? "default") !== "plan") {
                this.api.logger.debug?.(`codex turn request replacing active plan run ${this.formatConversationForLog(params.conversation)}`);
                this.activeRuns.delete(key);
                await existing.handle.interrupt().catch(() => undefined);
            }
            else if (!isQueueCompatibleTurnInput(params.prompt, params.input)) {
                this.api.logger.debug?.(`codex turn request restarting active run for structured input ${this.formatConversationForLog(params.conversation)} mode=${existing.mode}`);
                this.activeRuns.delete(key);
                await existing.handle.interrupt().catch(() => undefined);
            }
            else {
                try {
                    ...
                    if (handled) {
                        this.api.logger.debug?.(`codex turn request queued onto active run ${this.formatConversationForLog(params.conversation)} mode=${existing.mode}`);
                        return;
                    }
                    this.api.logger.warn(`codex turn request reached an active run but was not accepted; restarting ${this.formatConversationForLog(params.conversation)} mode=${existing.mode}`);
                }
                catch (error) {
                    this.api.logger.warn(`codex turn request active run enqueue failed; restarting ${this.formatConversationForLog(params.conversation)} mode=${existing.mode}: ${String(error)}`);
                }
                this.activeRuns.delete(key);
                await existing.handle.interrupt().catch(() => undefined);
            }
        ...
        this.api.logger.debug?.(`codex turn starting app-server run ${this.formatConversationForLog(params.conversation)} typing=${typing ? "yes" : "no"} session=${binding?.sessionKey ?? "<none>"} existingThread=${binding?.threadId ?? "<none>"} profile=${profile} mode=${params.collaborationMode?.mode ?? "default"}`);
        const desired = buildDesiredThreadConfiguration(undefined, binding, this.settings.defaultModel);
        const run = this.client.startTurn({
            ...
            onPendingInput: async (state) => {
                this.api.logger.debug?.(`codex turn pending input ${state ? "received" : "cleared"} ${this.formatConversationForLog(params.conversation)} questionnaire=${state?.questionnaire ? "yes" : "no"}`);
            ...
            onInterrupted: async () => {
                this.api.logger.debug?.(`codex turn interrupted ${this.formatConversationForLog(params.conversation)}`);
                await this.sendText(params.conversation, "Codex stopped.");
            },
        });
        this.api.logger.debug?.(`codex turn run handle created ${this.formatConversationForLog(params.conversation)}`);
        ...
        void run.result
            ...
            this.api.logger.debug?.(`codex turn completed ${this.formatConversationForLog(params.conversation)} thread=${threadId ?? "<none>"} aborted=${result.aborted ? "yes" : "no"} stoppedReason=${result.stoppedReason ?? "none"} terminalStatus=${result.terminalStatus ?? "none"} text=${result.text ? "yes" : "no"} plan=${result.planArtifact ? "yes" : "no"}`);
            const completionText = result.terminalStatus === "failed"
            ...
            .catch(async (error) => {
            const message = error instanceof Error ? error.message : String(error);
            this.api.logger.warn(`codex turn failed ${this.formatConversationForLog(params.conversation)}: ${message}`);
            await this.sendText(params.conversation, await this.describeTurnFailure({
                sessionKey: binding?.sessionKey,
                profile,
                error,
            }));
            ...
            this.api.logger.debug?.(`codex turn cleaned up ${this.formatConversationForLog(params.conversation)}`);
        });
    }
    async describeTurnFailure(params) {
        const message = params.terminalError?.message?.trim() ||
        ...
        if (this.looksLikeExplicitCodexAuthFailure(params.terminalError, message)) {
            const account = await this.client
                ...
                .catch(() => undefined);
            this.api.logger.warn?.(`codex auth failure from terminal turn error session=${params.sessionKey ?? "<none>"}: ${message}`);
        ...
        if (this.looksLikeCodexAuthFailure(message)) {
            const account = await this.client
                ...
                .catch(() => undefined);
            this.api.logger.warn?.(`codex auth failure inferred from turn error session=${params.sessionKey ?? "<none>"}: ${message}`);
            return this.formatCodexAuthFailureMessage(account);
        }
        return `Codex failed: ${message}`;
    }
    async describeEmptyTurnCompletion() {
        return "Codex completed without a text reply.";
    }
    formatCodexAuthFailureMessage(account) {
    ...
    looksLikeCodexAuthFailure(message) {
    ...
    looksLikeExplicitCodexAuthFailure(terminalError, message) {
    ...
    async startPlan(params) {
        ...
        if (params.announceStart !== false) {
            await this.sendText(params.conversation, "Starting Codex plan mode. I’ll relay the questions and final plan as they arrive.");
        }
        const typing = await this.startTypingLease(params.conversation);
        const threadState = params.binding?.threadId
            ? await this.client
                .readThreadState({
                profile,
        ...
        const desired = buildDesiredThreadConfiguration(threadState ?? undefined, params.binding, this.settings.defaultModel);
        ...
        const run = this.client.startTurn({
            ...
            onPendingInput: async (state) => {
                ...
                this.api.logger.debug(`codex plan pending input ${state ? `received (questionnaire=${state.questionnaire ? "yes" : "no"})` : "cleared"}`);
                await this.handlePendingInputState(params.conversation, params.workspaceDir, state, run);
            },
            onInterrupted: async () => {
    ...
    async startReview(params) {
        ...
        if (params.announceStart !== false) {
            await this.sendText(params.conversation, params.target.type === "custom"
                ? "Starting Codex review with your custom focus. I’ll send the findings when the review finishes."
                : "Starting Codex review of the current changes. I’ll send the findings when the review finishes.");
        }
        const typing = await this.startTypingLease(params.conversation);
        ...
        void run.result
            ...
            for (const [index, finding] of parsed.findings.entries()) {
                await this.sendText(params.conversation, formatCodexReviewFindingMessage({
                    finding,
                    index,
                }));
            }
            ...
            const allFixes = await this.store.putCallback({
                ...
                prompt: [
                    ...
                    "",
                    ...parsed.findings.map((finding, index) => `${index + 1}. ${finding.priorityLabel ? `[${finding.priorityLabel}] ` : ""}${finding.title}${finding.location ? `\n   ${finding.location}` : ""}${finding.body?.trim() ? `\n   ${finding.body.trim().replace(/\n/g, "\n   ")}` : ""}`),
                ].join("\n"),
            });
            buttons.push([
                {
            ...
            await this.sendText(params.conversation, "Choose a review finding to implement, or implement them all.", { buttons });
        })
            .catch(async (error) => {
    ...
    async handlePendingInputState(conversation, workspaceDir, state, run) {
        if (!state) {
        ...
        const callbacks = await Promise.all((state.actions ?? []).map(async (_action, actionIndex) => {
    ...
    buildQuestionnaireSubmissionPayload(pending) {
        ...
        return addQuestionnaireResponseNote(response, `This answer was selected by the user in chat after ${formatElapsedDuration(elapsedMs)}; it was not auto-selected.`);
    }
    async sendPendingQuestionnaire(conversation, state, opts) {
    ...
    async buildPendingQuestionnaireButtons(conversation, state) {
        const questionnaire = state.questionnaire;
        if (!questionnaire) {
            return [];
        }
    ...
    buildPendingButtons(state, callbacks) {
        ...
        for (let index = 0; index < actions.length; index += 2) {
            rows.push(actions.slice(index, index + 2).map((action, offset) => ({
                text: action.label,
    ...
    async handlePendingQuestionnaireFreeformAnswer(conversation, pending, run, text) {
        const questionnaire = pending.state.questionnaire;
        ...
        if (questionnaireIsComplete(questionnaire)) {
            const submitted = await run.submitPendingInputPayload(this.buildQuestionnaireSubmissionPayload(pending));
            if (!submitted) {
                return false;
            }
            await this.store.removePendingRequest(pending.requestId);
    ...
    resolveThreadWorkspaceDir(parsed, binding, useAllProjectsDefault) {
        if (parsed.cwd) {
            return parsed.cwd;
        }
    ...
    async resolveNewThreadWorkspaceDir(binding, parsed) {
        if (parsed.cwd) {
            return parsed.cwd;
        }
        ...
        const exactProjectName = listProjects(normalizedThreads).filter((project) => project.name.trim().toLowerCase() === query.toLowerCase());
    ...
    async listPickerThreads(binding, params) {
        const workspaceDir = this.resolveThreadWorkspaceDir(params.parsed, binding, params.filterProjectsOnly || Boolean(params.projectName));
        const profile = this.getPermissionsMode(binding);
    ...
    async normalizeNewThreadProjectThreads(threads) {
        const projectFolderByWorkspace = new Map();
        const getResolvedProjectFolder = (workspaceDir) => {
        ...
        const liveProjectRootsByName = new Map();
        for (const thread of threads) {
            const workspaceDir = thread.projectKey?.trim();
            ...
            const resolvedProjectRoot = this.isWorktreePath(workspaceDir)
                ...
                : workspaceDir;
            const projectRoots = liveProjectRootsByName.get(projectName) ?? new Set();
    ...
    async buildThreadPickerButtons(params) {
    ...
    async appendThreadPickerControls(params) {
    ...
    async renderThreadPicker(conversation, binding, parsed, page, projectName) {
        const profile = this.getPermissionsMode(binding);
        ...
        const distinctProjects = new Set(threads.map((thread) => getProjectName(thread.projectKey)).filter(Boolean));
        const threadButtons = (await this.buildThreadPickerButtons({
            conversation,
            parsed,
    ...
    async renderProjectPicker(conversation, binding, parsed, page, action = "resume-thread") {
        const { workspaceDir, threads } = await this.listPickerThreads(binding, {
            parsed,
            filterProjectsOnly: true,
        });
        const normalizedThreads = action === "start-new-thread" ? await this.normalizeNewThreadProjectThreads(threads) : threads;
        const buttons = [];
        ...
        for (const option of projectOptions.items) {
            const callback = action === "start-new-thread"
                ? (() => {
    ...
    async renderNewThreadWorkspacePicker(conversation, binding, parsed, page, projectName) {
        const { threads } = await this.listPickerThreads(binding, {
            parsed,
            projectName,
            filterProjectsOnly: true,
    ...
    async sendDiscordPicker(conversation, picker) {
        this.api.logger.debug(`codex discord picker send conversation=${conversation.conversationId} rows=${picker.buttons?.length ?? 0}`);
        ...
        const legacySend = this.api.runtime.channel.discord?.sendComponentMessage;
        if (typeof legacySend === "function") {
            await legacySend(conversation.conversationId, this.buildDiscordPickerSpec(picker), {
                accountId: conversation.accountId,
            });
            return;
        }
        ...
        if (typeof runtimeApi?.sendDiscordComponentMessage === "function") {
            await runtimeApi.sendDiscordComponentMessage(conversation.conversationId, this.buildDiscordPickerSpec(picker), {
                cfg: this.getOpenClawConfig(),
                accountId: conversation.accountId,
            });
            return;
    ...
    async sendDiscordPickerMessageLegacy(conversation, picker) {
        const legacySend = this.api.runtime.channel.discord?.sendComponentMessage;
        if (typeof legacySend === "function") {
            return await legacySend(conversation.conversationId, this.buildDiscordPickerSpec(picker), {
                accountId: conversation.accountId,
            });
        }
        const runtimeApi = await this.loadDiscordRuntimeApi();
        if (typeof runtimeApi?.sendDiscordComponentMessage === "function") {
            return await runtimeApi.sendDiscordComponentMessage(conversation.conversationId, this.buildDiscordPickerSpec(picker), {
                cfg: this.getOpenClawConfig(),
                accountId: conversation.accountId,
            });
        }
    ...
    async buildDiscordPickerMessage(picker) {
    ...
    async tryBuildDiscordPickerMessage(picker) {
        try {
    ...
    async loadDiscordSdk() {
        return await loadOpenClawCompatModule({
    ...
    async loadDiscordRuntimeApi() {
        try {
            const openClawEntrypointPath = resolveOpenClawEntrypointPath();
            const runtimeApiPath = resolveCompatFallbackPath(openClawEntrypointPath, "dist/extensions/discord/runtime-api.js");
            if (!existsSync(runtimeApiPath)) {
                return undefined;
            }
            return (await import(pathToFileURL(runtimeApiPath).href));
        }
        catch (error) {
    ...
    async loadDiscordExtensionApi() {
        try {
            const openClawEntrypointPath = resolveOpenClawEntrypointPath();
            const apiPath = resolveCompatFallbackPath(openClawEntrypointPath, "dist/extensions/discord/api.js");
    ...
    async editDiscordComponentMessage(to, messageId, spec, opts) {
        try {
            const discordSdk = await this.loadDiscordSdk();
    ...
    async registerBuiltDiscordComponentMessage(params) {
        try {
            const discordSdk = await this.loadDiscordSdk();
    ...
    async dispatchCallbackAction(callback, responders) {
        if (callback.kind === "start-new-thread") {
            if (responders.conversation.channel !== "discord") {
                await responders.clear().catch(() => undefined);
            }
            const result = await this.startNewThreadAndBindConversation(callback.conversation, this.store.getBinding(callback.conversation), callback.workspaceDir, callback.syncTopic ?? false, {
        ...
        if (callback.kind === "resume-thread") {
            ...
            const profile = this.resolveRequestedPermissionsMode(this.getPermissionsMode(currentBinding), callback.requestedYolo);
            const threadState = await this.client
                .readThreadState({
                profile,
                ...
                .catch(() => undefined);
            const preferences = this.buildBindingPreferencesWithOverrides(currentBinding?.preferences, {
                requestedModel: callback.requestedModel,
                requestedFast: callback.requestedFast,
                requestedYolo: undefined,
            }, callback.requestedModel ?? threadState?.model?.trim());
            const bindResult = await this.requestConversationBinding(callback.conversation, {
                threadId: callback.threadId,
        ...
        if (callback.kind === "pending-input") {
            ...
            if (!submitted) {
                await this.clearStaleInteractiveRun(callback.conversation, "pending-input action no longer available");
                await this.store.removeCallback(callback.token);
        ...
        if (callback.kind === "pending-questionnaire") {
            ...
            if (callback.action === "freeform") {
                questionnaire.currentIndex = Math.max(0, Math.min(callback.questionIndex, questionnaire.questions.length - 1));
                questionnaire.awaitingFreeform = true;
                pending.updatedAt = Date.now();
                await this.store.upsertPendingRequest(pending);
                await responders.reply(`Send a free-form answer for question ${questionnaire.currentIndex + 1} of ${questionnaire.questions.length} and I’ll record it.`);
            ...
            if (callback.action === "next") {
                ...
                questionnaire.currentIndex = Math.min(questionnaire.questions.length - 1, callback.questionIndex + 1);
                questionnaire.awaitingFreeform = false;
                pending.updatedAt = Date.now();
                await this.store.upsertPendingRequest(pending);
                await this.sendPendingQuestionnaire(callback.conversation, pending.state, {
            ...
            await this.store.upsertPendingRequest(pending);
            if (questionnaireIsComplete(questionnaire)) {
                const submitted = await active.handle.submitPendingInputPayload(this.buildQuestionnaireSubmissionPayload(pending));
                if (!submitted) {
                    await this.clearStaleInteractiveRun(callback.conversation, "pending-questionnaire submission no longer available");
                    await this.store.removeCallback(callback.token);
        ...
        if (callback.kind === "toggle-fast") {
            ...
            const currentModel = await this.resolveCurrentModelHint(binding, effectiveState);
            if (!modelSupportsFast(currentModel)) {
                await responders.reply(`Fast mode is unavailable for ${currentModel ?? "the current model"}. Use a GPT-5.4+ model to enable it.`);
                return;
            }
            const currentTier = normalizeServiceTier(effectiveState?.serviceTier ?? threadState?.serviceTier);
        ...
        if (callback.kind === "show-reasoning-picker") {
            ...
            const [picker, statusCard] = await Promise.all([
                this.buildReasoningPicker(conversation, binding, {
                    returnToStatus: true,
                }),
                this.buildStatusCard(conversation, binding, true),
            ]);
            await responders.editPicker({
                text: statusCard.text,
                buttons: picker.buttons,
        ...
        if (callback.kind === "set-reasoning") {
            ...
            await this.store.upsertBinding(updatedBinding);
            this.api.logger.debug?.(`codex status control set-reasoning conversation=${this.formatConversationForLog(callback.conversation)} requested=${normalizedReasoning} ${formatBindingPreferencesForLog(updatedBinding)}`);
        ...
        if (callback.kind === "toggle-permissions") {
            ...
            this.api.logger.debug?.(`codex status control toggle-permissions conversation=${this.formatConversationForLog(callback.conversation)} currentProfile=${currentProfile} requestedProfile=${nextProfile} activeRun=${active?.mode ?? "none"} ${formatBindingPreferencesForLog(updatedBinding)}`);
            const statusCard = await this.buildStatusCard({
                ...callback.conversation,
                threadId: responders.conversation.threadId,
            }, updatedBinding, true);
        ...
        if (callback.kind === "stop-run") {
            ...
            const active = this.activeRuns.get(buildConversationKey(callback.conversation));
            if (!active) {
                const cleared = await this.clearPendingRequestForConversation(callback.conversation, "status stop without active run");
                if (cleared) {
                    await this.applyPendingBindingPermissionsModeMigration(callback.conversation);
                }
                ...
                const statusCard = await this.buildStatusCard({
                    ...callback.conversation,
                    threadId: responders.conversation.threadId,
                }, nextBinding, Boolean(nextBinding));
                await responders.editPicker({
                    text: statusCard.text,
                    buttons: statusCard.buttons ?? [],
                });
        ...
        if (callback.kind === "run-skill") {
            ...
            const workspaceDir = callback.workspaceDir?.trim() ||
        ...
        if (callback.kind === "show-model-picker") {
            ...
            if (responders.sourceMessage) {
                const [picker, statusCard] = await Promise.all([
                    this.buildModelPicker(conversation, binding, {
                        returnToStatus: true,
                    }),
                    this.buildStatusCard(conversation, binding, true),
                ]);
        ...
        if (callback.kind === "set-model") {
            ...
            await this.store.upsertBinding(updatedBinding);
            this.api.logger.debug?.(`codex status control set-model conversation=${this.formatConversationForLog(callback.conversation)} requested=${callback.model} raw=${formatThreadStateForLog(nextState)} effective=${formatThreadStateForLog(applyBindingPreferencesToThreadState(nextState, updatedBinding))} ${formatBindingPreferencesForLog(updatedBinding)}`);
            if (callback.returnToStatus) {
                const statusCard = await this.buildStatusCard({
                    ...callback.conversation,
                    threadId: responders.conversation.threadId,
                ...
                if (callback.statusMessage) {
                    const updatedOriginal = await this.updateStatusCardMessage({
                        ...callback.conversation,
                        threadId: responders.conversation.threadId,
                    }, callback.statusMessage, statusCard);
                    if (updatedOriginal) {
                        await responders.editPicker({
        ...
        if (callback.kind === "rename-thread") {
            ...
            try {
                const name = await this.applyRenameStyle(responders.conversation, binding, callback.style, callback.syncTopic);
                await responders.reply(`Renamed the Codex thread to "${name}".`);
            }
            catch (error) {
                await responders.reply(error instanceof Error ? error.message : "Unable to derive a Codex thread name.");
            }
            return;
        }
        ...
        const binding = this.store.getBinding(callback.conversation);
        await this.store.removeCallback(callback.token);
        const parsed = callback.view.mode === "skills"
            ? null
            : {
                includeAll: callback.view.includeAll,
                listProjects: callback.view.mode === "projects",
                startNew: (callback.view.mode === "projects" && callback.view.action === "start-new-thread") ||
                ...
                query: callback.view.mode === "threads" || callback.view.mode === "projects"
                    ? callback.view.query ?? ""
                    : "",
            };
        const picker = callback.view.mode === "projects"
            ? await this.renderProjectPicker(responders.conversation, binding, parsed, callback.view.page, callback.view.action ?? "resume-thread")
            : callback.view.mode === "workspaces"
                ? await this.renderNewThreadWorkspacePicker(responders.conversation, binding, parsed, callback.view.page, callback.view.projectName)
                : callback.view.mode === "skills"
                    ? await this.buildSkillsPicker(responders.conversation, binding, {
                        page: callback.view.page,
                        filter: callback.view.filter,
                        clickMode: callback.view.clickMode,
                    })
                    : await this.renderThreadPicker(responders.conversation, binding, parsed, callback.view.page, callback.view.projectName);
        await responders.editPicker(picker);
    }
    async startNewThreadAndBindConversation(conversation, binding, workspaceDir, syncTopic, overrides, requestConversationBinding, notifyBound = true, followUpPrompt) {
        const profile = this.resolveRequestedPermissionsMode(this.getPermissionsMode(binding), overrides.requestedYolo);
        ...
        const preferences = this.buildBindingPreferencesWithOverrides(binding?.preferences, overrides, overrides.requestedModel ?? created.model?.trim());
        const bindResult = await this.requestConversationBinding(conversation, {
    ...
    async resolveSingleThread(sessionKey, workspaceDir, filter) {
    ...
    async persistBindingPermissionsMode(binding, profile, pendingProfile) {
        const nextBinding = {
            ...binding,
            permissionsMode: profile,
    ...
    async migrateBindingPermissionsMode(binding, profile) {
        ...
        const state = await this.client
            ...
            .catch(() => this.client.readThreadState({
            profile,
            sessionKey: binding.sessionKey,
            threadId: binding.threadId,
        }));
    ...
    async applyPendingBindingPermissionsModeMigration(conversation) {
        ...
        try {
            ...
            this.api.logger.debug?.(`codex migrated binding profile ${this.formatConversationForLog(conversation)} profile=${pendingProfile}`);
            return migrated;
        }
        catch (error) {
            this.api.logger.warn(`codex failed to migrate binding profile ${this.formatConversationForLog(conversation)} target=${pendingProfile}: ${String(error)}`);
            return binding;
        }
    }
    async bindConversation(conversation, params) {
    ...
    async hydrateApprovedBinding(conversation) {
        const existing = this.store.getBinding(conversation);
        if (existing) {
    ...
    async requestConversationBinding(conversation, params, requestBinding) {
        if (!requestBinding) {
            return {
                status: "error",
    ...
    trimReplayText(value, maxLength = 1200) {
        const trimmed = value?.trim();
        if (!trimmed) {
            return undefined;
        }
    ...
    isWorktreePath(projectKey) {
        const trimmed = projectKey?.trim();
        return Boolean(trimmed && /[/\\]worktrees[/\\][^/\\]+[/\\][^/\\]+/.test(trimmed));
    }
    readThreadHasChanges(projectKey) {
    ...
    async buildBoundConversationMessages(conversation) {
        ...
        const readStateForRestore = async () => {
            try {
                ...
                if (isMissingThreadError(error)) {
                    this.api.logger.warn(`codex bound restore could not read thread state ${this.formatConversationForLog(restoreConversation)} boundThread=${binding.threadId}: ${String(error)}`);
                    return undefined;
                }
                throw error;
            }
        };
        const readReplayForRestore = async () => {
            try {
                ...
                if (isMissingThreadError(error)) {
                    this.api.logger.warn(`codex bound restore could not read thread replay ${this.formatConversationForLog(restoreConversation)} boundThread=${binding.threadId}: ${String(error)}`);
        ...
        const state = (await this.reconcileThreadConfiguration(binding, {
            threadState: initialState,
            context: "restore desired thread settings",
        })) ?? initialState;
        const syncedBinding = state
            ? (await this.syncBindingPermissionsModeFromThreadState(binding, state, "build bound conversation messages")).binding
            : binding;
        const nextBinding = (state?.threadName && state.threadName !== syncedBinding.threadTitle) ||
    ...
    async sendBoundConversationSummary(conversation) {
    ...
    async sendBoundConversationNotifications(conversation) {
        const target = {
            channel: conversation.channel,
            accountId: conversation.accountId,
            conversationId: conversation.conversationId,
    ...
    async sendStatusCardCommandReply(conversation, text, buttons) {
        try {
    ...
    async buildStatusText(conversation, binding, bindingActive) {
        const activeRun = bindingActive && conversation
        ...
        let permissionsRefreshNote;
        if (binding && threadState) {
            const synced = await this.syncBindingPermissionsModeFromThreadState(binding, threadState, "build status text");
        ...
        const effectiveThreadState = buildDesiredThreadConfiguration(threadState, effectiveBinding).effectiveState;
        const displayThreadState = effectiveThreadState ??
            (effectiveBinding
                ? {
                ...
                : undefined);
        const threadNote = effectiveBinding && !threadState
        ...
        return formatCodexStatusText({
            ...
            bindingActive,
            projectFolder,
            worktreeFolder: displayThreadState?.cwd?.trim() || effectiveBinding?.workspaceDir || workspaceDir,
    ...
    async buildPlanDelivery(plan) {
        ...
        const attachmentPath = path.join(tempDir, `codex-plan-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}.md`);
    ...
    async sendReply(conversation, payload) {
        const delivered = await this.sendReplyWithDeliveryRef(conversation, payload);
        return delivered !== null;
    }
    async sendReplyWithDeliveryRef(conversation, payload) {
        const text = payload.text?.trim() ?? "";
        const hasMedia = typeof payload.mediaUrl === "string" && payload.mediaUrl.trim().length > 0;
        if (!text && !hasMedia) {
            return null;
        }
        this.api.logger.debug?.(`codex outbound send start ${this.formatConversationForLog(conversation)} textChars=${text.length} media=${hasMedia ? "yes" : "no"} buttons=${payload.buttons?.length ?? 0} preview="${summarizeTextForLog(text, 80)}"`);
        if (isTelegramChannel(conversation.channel)) {
            ...
            const limit = this.api.runtime.channel.text.resolveTextChunkLimit(undefined, "telegram", conversation.accountId, { fallbackLimit: 4000 });
            const chunks = text
                ? this.api.runtime.channel.text.chunkText(text, limit).filter(Boolean)
                : [];
            let delivered = null;
            if (hasMedia) {
                const result = chunks.length <= 1 && payload.buttons && outbound?.sendPayload
                    ? await outbound.sendPayload({
                        cfg: this.getOpenClawConfig(),
                ...
                delivered = {
                    ...
                    chatId: typeof result.chatId === "string"
                        ? result.chatId
                ...
                for (let index = 1; index < chunks.length; index += 1) {
                    ...
                    const result = index === chunks.length - 1 && payload.buttons && outbound?.sendPayload
                        ? await outbound.sendPayload({
                            cfg: this.getOpenClawConfig(),
                            to: conversation.parentConversationId ?? conversation.conversationId,
                            accountId: conversation.accountId,
                ...
                this.api.logger.debug?.(`codex outbound send complete ${this.formatConversationForLog(conversation)} channel=telegram chunks=${Math.max(chunks.length, 1)} media=${hasMedia ? "yes" : "no"}`);
                return delivered;
            }
            ...
            for (let index = 0; index < textChunks.length; index += 1) {
                ...
                const result = index === textChunks.length - 1 && payload.buttons && outbound?.sendPayload
                    ? await outbound.sendPayload({
                        cfg: this.getOpenClawConfig(),
                        to: conversation.parentConversationId ?? conversation.conversationId,
                        accountId: conversation.accountId,
            ...
            this.api.logger.debug?.(`codex outbound send complete ${this.formatConversationForLog(conversation)} channel=telegram chunks=${textChunks.length} media=no`);
            return delivered;
        }
        if (isDiscordChannel(conversation.channel)) {
            const outbound = await this.loadDiscordOutboundAdapter();
            const mediaLocalRoots = this.resolveReplyMediaLocalRoots(payload.mediaUrl);
            const limit = this.api.runtime.channel.text.resolveTextChunkLimit(undefined, "discord", conversation.accountId, { fallbackLimit: 2000 });
            const chunks = text
                ? this.api.runtime.channel.text.chunkText(text, limit).filter(Boolean)
                : [];
            let delivered = null;
            if (payload.buttons && payload.buttons.length > 0) {
                this.api.logger.debug(`codex discord reply send conversation=${conversation.conversationId} rows=${payload.buttons.length}`);
                ...
                if (hasMedia) {
                    ...
                    delivered = {
                        provider: "discord",
                        messageId: result.messageId,
                        channelId: typeof result.channelId === "string"
                            ? result.channelId
                ...
                this.api.logger.debug?.(`codex outbound send complete ${this.formatConversationForLog(conversation)} channel=discord chunks=${chunks.length + 1 + (hasMedia ? 1 : 0)} media=${hasMedia ? "yes" : "no"} buttons=${payload.buttons.length}`);
                return delivered;
            }
            const textChunks = chunks.length > 0 ? chunks : [text];
            if (hasMedia) {
                ...
                delivered = {
                    provider: "discord",
                    messageId: result.messageId,
                    channelId: typeof result.channelId === "string" ? result.channelId : conversation.conversationId,
                };
            }
            ...
            this.api.logger.debug?.(`codex outbound send complete ${this.formatConversationForLog(conversation)} channel=discord chunks=${textChunks.length + (hasMedia ? 1 : 0)} media=${hasMedia ? "yes" : "no"}`);
            return delivered;
        }
        return null;
    }
    ...
    async loadTelegramOutboundAdapter() {
        const loadAdapter = this.api.runtime.channel.outbound?.loadAdapter;
        if (typeof loadAdapter !== "function") {
            return undefined;
        }
        return (await loadAdapter("telegram"));
    }
    async loadDiscordOutboundAdapter() {
        const loadAdapter = this.api.runtime.channel.outbound?.loadAdapter;
        if (typeof loadAdapter !== "function") {
            return undefined;
        }
        return (await loadAdapter("discord"));
    }
    async sendTelegramTextChunk(outbound, conversation, text, opts) {
    ...
    async sendTelegramMediaChunk(outbound, conversation, text, opts) {
        if (!opts.mediaUrl) {
    ...
    async sendDiscordTextChunk(outbound, conversation, text, opts) {
        ...
        const legacySend = this.api.runtime.channel.discord?.sendMessageDiscord;
        if (typeof legacySend === "function") {
    ...
    resolveReplyMediaLocalRoots(mediaUrl) {
        ...
        const roots = new Set([this.api.runtime.state.resolveStateDir(), path.dirname(localPath)]);
        return [...roots];
    }
    buildRunPromptAckText(prompt) {
    ...
    async resolveProjectFolder(worktreeFolder) {
        const cwd = worktreeFolder?.trim();
        if (!cwd) {
            return undefined;
        }
        try {
            const result = await execFileAsync("git", ["-C", cwd, "rev-parse", "--path-format=absolute", "--git-common-dir"], { timeout: 5_000 });
    ...
    async unbindConversation(conversation) {
        const binding = this.store.getBinding(conversation);
    ...
    async startTypingLease(conversation) {
        if (isTelegramChannel(conversation.channel)) {
        ...
        if (isDiscordChannel(conversation.channel)) {
            ...
            const channelId = denormalizeDiscordConversationId(conversation.conversationId) ?? conversation.conversationId;
            const legacyTyping = this.api.runtime.channel.discord?.typing?.start;
            if (typeof legacyTyping !== "function") {
                const runtimeApi = await this.loadDiscordRuntimeApi();
    ...
    async sendText(conversation, text, opts) {
        await this.sendReply(conversation, {
            text,
            buttons: opts?.buttons,
        });
    }
    async sendTextWithDeliveryRef(conversation, text) {
        const trimmed = text.trim();
        if (!trimmed) {
            return null;
        }
    ...
    async pinBindingMessage(conversation, delivered) {
        if (!delivered) {
            return;
        }
        const binding = this.store.getBinding(conversation);
        ...
        try {
            if (delivered.provider === "telegram") {
                const token = await this.resolveTelegramBotToken(conversation.accountId);
                if (!token) {
                    this.api.logger.debug?.(`codex telegram pin skipped ${this.formatConversationForLog(conversation)} reason=no-token`);
                    return;
                }
                ...
                if (!token) {
                    this.api.logger.debug?.(`codex discord pin skipped ${this.formatConversationForLog(conversation)} reason=no-token`);
                    return;
                }
    ...
    async unpinStoredBindingMessage(binding) {
        ...
        try {
            if (pinned.provider === "telegram") {
                ...
                if (!token) {
                    this.api.logger.debug?.(`codex telegram unpin skipped conversation=${binding.conversation.conversationId} reason=no-token`);
                    return;
                }
                ...
                if (!token) {
                    this.api.logger.debug?.(`codex discord unpin skipped conversation=${binding.conversation.conversationId} reason=no-token`);
                    return;
                }
    ...
    async resolveTelegramBotToken(accountId) {
        const legacyResolution = this.api.runtime.channel.telegram?.resolveTelegramToken?.(this.getOpenClawConfig(), { accountId });
    ...
    async resolveDiscordBotToken(accountId) {
    ...
    async startTelegramTypingLease(conversation) {
        const token = await this.resolveTelegramBotToken(conversation.accountId);
        if (!token) {
            return null;
        }
        ...
        const sendTyping = async () => {
            ...
            if (!response.ok) {
                throw new Error(`Telegram sendChatAction failed status=${response.status} body=${await response.text()}`);
            }
        };
        await sendTyping().catch((error) => {
    ...
    async callDiscordPinApi(action, token, channelId, messageId) {
        const response = await fetch(`https://discord.com/api/v10/channels/${encodeURIComponent(channelId)}/pins/${encodeURIComponent(messageId)}`, {
        ...
        if (!response.ok) {
            throw new Error(`Discord ${action} failed status=${response.status} body=${await response.text()}`);
        }
    }
    async callTelegramBotApi(method, token, body) {
    ...
    async callTelegramPinApi(method, token, body) {
        await this.callTelegramBotApi(method, token, body);
    }
    async callTelegramEditMessageApi(token, body) {
        await this.callTelegramBotApi("editMessageText", token, body);
    }
    async callTelegramTopicEditApi(token, body) {
        await this.callTelegramBotApi("editForumTopic", token, body);
    }
    async renameConversationIfSupported(conversation, name) {
        if (isTelegramChannel(conversation.channel) && conversation.threadId != null) {
            ...
            if (typeof legacyRename === "function") {
                await legacyRename(conversation.parentConversationId ?? conversation.conversationId, conversation.threadId, name, {
                    accountId: conversation.accountId,
                }).catch((error) => {
        ...
        if (isDiscordChannel(conversation.channel)) {
            const legacyEditChannel = this.api.runtime.channel.discord?.conversationActions?.editChannel;
            if (typeof legacyEditChannel !== "function") {
                ...
                await runtimeApi.editChannelDiscord({
                    channelId: denormalizeDiscordConversationId(conversation.conversationId) ??
                        conversation.conversationId,
                    name,
                }, {
                    cfg: this.getOpenClawConfig(),
            ...
            await legacyEditChannel(conversation.conversationId, {
                name,
            }, {
                accountId: conversation.accountId,
            }).catch((error) => {
